!(function (o) {
  ...
  (e.prototype.onDrop = function (e, t) {
    ...
    (e.prototype.onEventClick = function (t, e, n) {
      var a = this,
        l = o("<form></form>");
      l.append("<label>휴관일명</label>"),
        l.append(
          "<div class='input-group m-b-15'><input class='form-control' type=text value='" +
            t.title +
            "' /><span class='input-group-append'><button type='submit' class='btn btn-success btn-md  '><i class='fa fa-check'></i> 저장</button></span></div>"
        ),
    ...
    (e.prototype.onSelect = function (n, a, e) {
      ...
      i.append("<div class='row'></div>"),
        i
          .find(".row")
          .append(
            "<div class='col-12'><div class='form-group'><label class='control-label'>휴관일명</label><input class='form-control' type='text' name='title'/></div></div>"
          ),
          /*.append(
            "<div class='col-12'><div class='form-group'><label class='control-label'>카테고리</label><select class='form-control' name='category'></select></div></div>"
          )
          .find("select[name='category']")
          .append("<option value='bg-info'>행사</option>")
          .append("<option value='bg-success'>교직원 교육</option>")
          .append("<option value='bg-primary'>Primary</option>")
          .append("<option value='bg-warning'>Info</option>")
          .append("<option value='bg-dark'>Dark</option>") 
          .append("<option value='bg-danger'>휴관일</option></div></div>"),*/
        l.$modal
          .find(".delete-event")
          .hide()
          .end()
