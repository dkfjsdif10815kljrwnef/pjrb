      ...
      <div class="card">
        <div class="card-body">
		  <ul class="nav nav-tabs nav-bordered mb-4">
            <li class="nav-item">
              <a href="javascript:void(0);" onclick="javascript:fn_searchType('');" class="nav-link <c:if test="${searchVO.searchType == null || searchVO.searchType eq '' }">active</c:if>">전체</a>
            </li>
            <li class="nav-item">
              <a href="javascript:void(0);" onclick="javascript:fn_searchType('admin');" class="nav-link <c:if test="${searchVO.searchType eq 'admin' }">active</c:if>">관리자</a>
            </li>
            <li class="nav-item">
              <a href="javascript:void(0);" onclick="javascript:fn_searchType('general');" class="nav-link <c:if test="${searchVO.searchType eq 'general' }">active</c:if>">일반회원</a>
            </li>
            <li class="nav-item">
              <a href="javascript:void(0);" onclick="javascript:fn_searchType('orgnzt');" class="nav-link <c:if test="${searchVO.searchType eq 'orgnzt' }">active</c:if>">기관회원</a>
            </li>
          </ul> <!-- end nav-->
          ...
          <form method="post" id="frm" name="frm">
              ...
              <div class="dt-buttons col-sm-12 mb-3">
                <button class="btn btn-secondary flaot-left" type="button" onclick="javascript:fn_excel('member','<c:out value='${searchVO.searchType}'/>');">
                	<span>엑셀 다운로드</span>
                </button>
                  ...
                  <tbody>
                  	<c:forEach var="result" items="${resultList}" varStatus="status">
	                    <tr>
	                      ...
	                      <c:if test="${searchVO.searchType eq 'orgnzt' }">
		                      <td>
		                        <input type="checkbox" id="allowYn${status.index }" <c:if test="${result.allowYn eq 'Y' }">checked</c:if> data-switch="bool" onclick="javascript:fn_memberAllowYnUpdate('<c:out value="${result.emplyrId }"/>','<c:out value="${status.index }"/>');"/>
		                        <label for="allowYn${status.index }" data-on-label="사용" data-off-label="미사용"></label>
		                      </td>
		                  </c:if>
	                      <c:if test="${searchVO.searchType eq 'admin' }">
		                      <td>
		                        <select class="form-control" id="grade" onchange="javascript:fn_gradeChange('<c:out value="${result.emplyrId }"/>', this)">
		                        	<option value="">권한 선택</option>
		                        	<c:forEach var="grade" items="${gradeList}" varStatus="status2">
		                        		<option value="<c:out value="${grade.grade }"/>" <c:if test="${grade.grade eq result.grade }">selected</c:if>>
