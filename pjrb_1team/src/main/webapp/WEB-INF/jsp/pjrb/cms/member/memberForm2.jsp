	...
	<div class="row">
		<div class="col-lg-6 form-row">
		    <div class="form-group col-lg-5">
		        <label><i class="fad fa-check"></i>기관구분</label>
				<select class="form-control" required name="orgnztFlag" onchange="javascript:fn_orgnztFlag(this);">
					<option value="">선택</option>
					<option value="1" <c:if test="${memberView.orgnztFlag eq '1' }">selected</c:if>>국공립</option>
                    <option value="2" <c:if test="${memberView.orgnztFlag eq '2' }">selected</c:if>>사회복지법인</option>
                    <option value="3" <c:if test="${memberView.orgnztFlag eq '3' }">selected</c:if>>법인 단체 등</option>
                    <option value="4" <c:if test="${memberView.orgnztFlag eq '4' }">selected</c:if>>민간</option>
                    <option value="5" <c:if test="${memberView.orgnztFlag eq '5' }">selected</c:if>>가정</option>
                    <option value="6" <c:if test="${memberView.orgnztFlag eq '6' }">selected</c:if>>협동</option>
                    <option value="7" <c:if test="${memberView.orgnztFlag eq '7' }">selected</c:if>>기타</option>
				</select>
		    </div>
		    <div class="form-group col-lg-7">
		    	<label>&nbsp;</label>
				<input type="text" class="form-control col-lg-12" id="orgnztFlagDetail" placeholder="기타 내용을 입력해 주세요" value="<c:out value="${memberView.orgnztFlagDetail }"/>" <c:if test="${memberView.orgnztFlag ne '7' }">readonly</c:if> name="orgnztFlagDetail">
		    </div>
		</div>
		<div class="col-lg-6">
			<div class="form-group">
				<label><i class="fad fa-check"></i>기관명</label>
				<input type="text" class="form-control" value="<c:out value="${memberView.orgnztNm }"/>" required name="orgnztNm">
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-4">
			<div class="form-group">
				<label><i class="fad fa-check"></i>대표자명</label>
				<input type="text" class="form-control" value="<c:out value="${memberView.ceoNm }"/>" required name="ceoNm">
			</div>
		</div>
		<div class="col-lg-4">
			<div class="form-group">
				<label><i class="fad fa-check"></i>사업자번호</label>
				<div class="input-group">
					<input type="text" class="form-control company_no" data-threshold="12" maxlength="12" data-toggle="maxlength" value="<c:out value="${memberView.businessNum }"/>" required name="businessNum" <c:if test="${memberView.emplyrId != null && memberView.emplyrId ne '' }">readonly</c:if>>
					<c:if test="${memberView.emplyrId == null || memberView.emplyrId eq '' }">
						<div class="input-group-append">
							<button class="btn btn-light" type="button" onclick="javascript:fn_bisNumChk();">중복체크</button>
						</div>
					</c:if>
				</div>
			</div>
		...
		<div class="col-lg-4">
			<div class="form-group">
				<label><i class="fad fa-check"></i>대표번호</label>
				<input type="text" class="form-control phoneNumber" data-threshold="13" maxlength="13" data-toggle="maxlength" required name="orgnztNum" value="<c:out value="${memberView.orgnztNum }"/>">
			</div>
		</div>
	</div>
	<div class="row">
	...
	<div class="row">
		...
		<div class="col-lg-1">
			<div class="form-group">
				<label>승인여부</label>
				<div>
					<input type="checkbox" id="switch3" <c:if test="${memberView.allowYn eq 'Y' }">checked</c:if> data-switch="bool" />
					<label for="switch3" data-on-label="승인" data-off-label="미승인"></label>
				</div>
			</div>
		</div>
		<div class="col-lg-1">
