...
public class PageMngExcelDownMapping extends AbstractPOIExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model, XSSFWorkbook workbook, HttpServletRequest request,
		...
		for (int i = 0; i < resultList.size(); i++) {			
			...
			if(tmp.get("pageNm") != null && !tmp.get("pageNm").toString().isEmpty()){
				cell = getCell(sheet, rowIdx, cellIdx++);
				cell.setCellStyle(mainCs);
				setText(cell,tmp.get("pageNm").toString());
			}else{
				cell = getCell(sheet, rowIdx, cellIdx++);
				cell.setCellStyle(mainCs);
				setText(cell,"");
			...
			if(tmp.get("cdate") != null && !tmp.get("cdate").toString().isEmpty()){
				cell = getCell(sheet, rowIdx, cellIdx++);
				cell.setCellStyle(mainCs);
				setText(cell,tmp.get("cdate").toString().substring(0, 10));
			}else{
				cell = getCell(sheet, rowIdx, cellIdx++);
				cell.setCellStyle(mainCs);
				setText(cell,"");
			...
			/*if(tmp.get("useYn") != null && !tmp.get("useYn").toString().isEmpty()){
				String useYn = tmp.get("useYn").toString();
				if("Y".equals(useYn)){
					cell = getCell(sheet, rowIdx, cellIdx++);
					cell.setCellStyle(mainCs);
					setText(cell,"사용");
